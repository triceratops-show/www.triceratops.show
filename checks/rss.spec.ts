...
const rss = await readFile(config.rssFilePath)
...
expect(lastestEpisode.enclosure['@_length']).to.be.a('string').that.is.not.empty;
...
expect(lastestEpisode['itunes:image']['@_href']).to.be.a('string').that.is.not.empty;
...
expect(res.headers['content-length']).to.equal(lastestEpisode.enclosure['@_length']);
